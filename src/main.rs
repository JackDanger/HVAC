...
#[derive(Parser, Debug)]
#[command(name = "hvecuum", version, about = "GPU-accelerated media transcoder")]
struct Cli {
...
fn embedded_config_banner(use_unicode: bool) -> String {
    ...
    let content: &[&str] = &[
        "  hvecuum: using built-in default configuration",
        "",
        "  To customise encoding settings, save the defaults to a file:",
        "",
        "    hvecuum --dump-config > config.yaml",
        "    $EDITOR config.yaml",
        "    hvecuum --config config.yaml /path/to/media",
        "",
        "  Suppress this message: hvecuum --quiet ...",
    ];
...
fn main() -> Result<()> {
    ...
    // Clean up any leftover .hvecuum_tmp_* files
    cleanup_tmp_dirs();
...
/// Remove .hvecuum_tmp_* files from all registered work directories.
fn cleanup_tmp_dirs() {
    ...
    if let Ok(entries) = std::fs::read_dir(dir) {
        for entry in entries.flatten() {
            if let Some(name) = entry.file_name().to_str() {
                if name.starts_with(".hvecuum_tmp_") {
